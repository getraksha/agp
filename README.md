...
- **Credential isolation, including OAuth.** Agents never hold tool credentials. The proxy injects them at request time — and for MCP servers behind OAuth (Slack, GitHub, Calendly, Notion, …), **connectors** run the consent flow and let AGP hold the credential, supporting pre-registered OAuth clients *and* Dynamic Client Registration (DCR), then minting a short-lived, scoped credential per call. A compromised or manipulated agent has nothing to exfiltrate.
...
| **Connectors** | OAuth consent + credential lifecycle for MCP servers behind OAuth — pre-registered clients or Dynamic Client Registration (DCR) |
...
# 4. Start the stack (10 services, health-checked)
agp start all
agp status
